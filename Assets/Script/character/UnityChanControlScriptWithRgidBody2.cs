...
namespace UnityChan
{
    ...
    public class UnityChanControlScriptWithRgidBody2 : MonoBehaviour
    {
        ...
        public bool whichLeft { get; private set; } = false;
        ...
        void Start()
        {
            player = GetComponent<Kajitani.Player>();
        ...
        {
            //ユーザーモードでなければ行わない
            if (player.type != Player.Type.UserMode)
            {
                return;
            }
            ...
            Debug.DrawRay(transform.position, Vector3.down, Color.red, 10, true);
        ...
        void Update()
        {
            //        // Jump bool値をリセットする（ループしないようにする）				
                    anim.SetBool("Jump", false);
            //FixedUpdateではトリガーがうまくいかないようなので
            if (Input.GetButtonDown("Jump"))
            { // スペースキーを入力したら

                if (Physics.Raycast(transform.position + Vector3.up, Vector3.down, 1.0f))
                {
